...
const borrow_key: oper_key = (status:Key) => {return status==='RETURN' ? 'BORROW' : status}//鍵を借りることができるかどうかの判定.0なら成功で1を返し, 失敗なら引数の値をそのまま返す.
const open_key: oper_key = (status:Key) => {return (status==='BORROW' || status==='CLOSE') && !mode_console ? 'OPEN' : status}//鍵を開けることができるかどうかの判定.1か3なら成功で2を返し, 失敗なら引数の値をそのまま返す.操作卓モードだと失敗する.
const close_key: oper_key = (status:Key) => {return status==='OPEN' && !mode_console ? 'CLOSE' : status}//鍵を閉めることができるかどうかの判定.2なら成功で3を返し, 失敗なら引数の値をそのまま返す.操作卓モードだと失敗する.
const return_key: oper_key = (status:Key) => {return status==='BORROW' || status==='CLOSE' ? 'RETURN' : status}//鍵を返却することができるかどうかの判定.1か3なら成功で3を返し, 失敗なら引数の値をそのまま返す.


// ボタンを定義している
const borrow_button = new ButtonBuilder().setCustomId('BORROW').setLabel('借りる').setStyle(ButtonStyle.Success)
const opne_button = new ButtonBuilder().setCustomId('OPEN').setLabel('開ける').setStyle(ButtonStyle.Success)
const close_button = new ButtonBuilder().setCustomId('CLOSE').setLabel('閉める').setStyle(ButtonStyle.Danger)
const return_button = new ButtonBuilder().setCustomId('RETURN').setLabel('返す').setStyle(ButtonStyle.Danger)
...
const mapButtons:Map<Key,ActionRowBuilder<ButtonBuilder>> = new Map([
    ['RETURN', new ActionRowBuilder<ButtonBuilder>().addComponents(borrow_button)],
    ['BORROW', !mode_console ? new ActionRowBuilder<ButtonBuilder>().addComponents(opne_button).addComponents(return_button) : new ActionRowBuilder<ButtonBuilder>().addComponents(return_button)],
    ['OPEN', new ActionRowBuilder<ButtonBuilder>().addComponents(close_button)],
    ['CLOSE', new ActionRowBuilder<ButtonBuilder>().addComponents(return_button).addComponents(opne_button)],
])
...
client.once('ready',(bot) => {
    ...
    //鍵用チャンネルに初期メッセージを送る
    if(id_log_channel) {
        const initialButtonSet:ActionRowBuilder<ButtonBuilder> = mapButtons.get('RETURN') ?? new ActionRowBuilder<ButtonBuilder>;
...
const isKey = (value: string): value is Key => {
    return value === 'BORROW' || value ==='OPEN' || value === 'CLOSE' || value === 'RETURN'
}

client.on(Events.InteractionCreate, async (interaction) => {
    if (!interaction.isButton()) {throw Error('interaction is not Button')}//インタラクションがボタンかどうかを確認する
    if (!isKey(var_status)) {throw Error('var_status is not apropriate')}//var_statusの型がKeyかどうかを確認する
    
    const btn = interaction.customId//押されたボタンの状態(型:Key)を代入する
    if (!isKey(btn)) {throw Error('buttonInteraction.customId is not Key')}//customIdがKey型かどうかを確認する.

    const oper = mapOpers.get(btn)//押されたボタンに対応する操作を得る
    if (!oper) {throw Error('oper is undefined')}
    var_status = oper(var_status)//状態を更新する

    const buttonSet = mapButtons.get(var_status)//更新後の状態に対応するボタンセットを得る
    if (!buttonSet) {throw Error('buttonSet is undefined')}

    const label = mapLabel.get(var_status)//更新後の状態に対応するラベルを得る
    if (!label) {throw Error('label is undefined')}
